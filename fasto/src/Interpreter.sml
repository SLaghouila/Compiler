...
fun evalExp ( Constant (v,_), vtab, ftab ) = v
  ...
  | evalExp ( Negate(e1, pos), vtab, ftab ) =
        let val res1   = evalExp(e1, vtab, ftab)
            in  evalBinopNum(op ~, res1, pos)
        end
  | evalExp ( Not(e1, pos), vtab, ftab ) =
       let Boolval res1  = evalExp(e1, vtab, ftab)
            in case res1 of
              True => False
            | False => True
            | _ => raise Error("Argument not boolian")
        end
  | evalExp ( And(e1, e2, pos), vtab, ftab ) =
        let Boolval res1   = evalExp(e1, vtab, ftab)
            Boolval res2   = evalExp(e2, vtab, ftab)
        in  evalEq(res1, res2, pos)
        end
